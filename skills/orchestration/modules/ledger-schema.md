...
| `batch.meta` | `batch`, plus fields | Merges fields (e.g. `pr`, `branch`, `head_sha`, a reduced `issues`, a forgiven `attempts`) onto the batch. `batch.py paths --apply` writes `paths` (the branch's real diff) and `paths_head` (the commit it was of); `land.merge_blockers` refuses a batch without the second, or whose `paths_head` is not the commit being merged. **It cannot set `state`, `ci_gate`, `review_gate` or the clocks** — those are dropped, because each has an event of its own whose validation is what stands between the loop and an unreviewed merge. |
