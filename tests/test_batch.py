...
def rec(n, risk="low", size="small", paths=()):
    return {"issue": n, "verdict": "actionable", "risk": risk, "size": size,
            "paths": list(paths), "title": f"issue {n}"}
...
def test_compatible_issues_are_packed_together():
    groups = batch.group_issues([rec(1, paths=["a.py"]), rec(2, paths=["b.py"]),
                                 rec(3, paths=["c.py"])], CONFIG)
    assert len(groups) == 1
...
def test_the_weight_cap_closes_a_batch_before_the_issue_cap():
    records = [rec(1, size="medium", paths=["a.py"]), rec(2, size="medium", paths=["b.py"]),
...
def test_a_high_risk_issue_gets_a_batch_of_its_own_rather_than_being_dropped():
    groups = batch.group_issues([rec(1, paths=["a.py"]), rec(2, risk="high", paths=["b.py"])],
                                CONFIG)
...
def test_a_batch_carries_the_union_of_its_paths_and_its_worst_risk():
    groups = batch.group_issues([rec(1, paths=["a.py"]), rec(2, risk="medium", paths=["b.py"])],
                                CONFIG)
...
def test_no_savings_claimed_when_every_batch_holds_one_issue():
    saved = batch.estimate_savings([{"issues": [1]}, {"issues": [2]}],
                                   {"cheap_tier_s": 60, "expensive_tier_s": 60})
...
def test_a_failing_issue_peels_off_and_the_rest_proceed():
    failing, rest = batch.split({"id": "b-001", "issues": [1, 2, 3], "risk": "low",
                                 "paths": ["a.py"]}, failing_issue=2)
